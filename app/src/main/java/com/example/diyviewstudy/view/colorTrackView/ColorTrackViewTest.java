...
public class ColorTrackViewTest extends AppCompatActivity {
    ...
    private ColorTrackTextView tab_home;
    private ColorTrackTextView tab_mine;
    ...
    private void setUpWithViewPager(ViewPager viewPager, final LinearLayout ll_tablayout) {
        ...
        ((ColorTrackTextView) ll_tablayout.getChildAt(viewPager.getCurrentItem())).setProgress(1);
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            @Override
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                Log.e("setUpWithViewPager", "onPageScrolled: " + position + "," + positionOffset + "," + positionOffsetPixels);

                ColorTrackTextView currentTab = (ColorTrackTextView) ll_tablayout.getChildAt(position);
                currentTab.setProgress(positionOffset - 1);
                if ((position + 1) < ll_tablayout.getChildCount()) {
                    ColorTrackTextView nextTab = (ColorTrackTextView) ll_tablayout.getChildAt(position + 1);
